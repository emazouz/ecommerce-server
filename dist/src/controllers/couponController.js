...
exports.applyCoupon = exports.validateCoupon = exports.deleteCoupon = exports.updateCoupon = exports.getCouponById = exports.getPublicCoupons = exports.fetchAllCoupons = exports.createCoupon = void 0;
...
const createCoupon = async (req, res) => {
    try {
        ...
        const { code, description, discountType, discountValue, maxDiscount, startDate, endDate, isActive, isPublic, maxUsage, maxUsagePerUser, minOrderValue, productTypeId, categoryIds, excludedProductIds, userIds, } = req.body;
        // Create coupon with relationships
        const newCoupon = await prisma_1.prisma.coupon.create({
            data: {
                ...
                maxUsagePerUser: maxUsagePerUser !== undefined ? parseInt(maxUsagePerUser) : 1,
                usedCount: 0,
                minOrderValue: minOrderValue !== undefined ? parseFloat(minOrderValue) : 0,
                ...
                excludedProducts: excludedProductIds && excludedProductIds.length > 0
                    ? {
...
const fetchAllCoupons = async (req, res) => {
    try {
        ...
        const totalCoupons = await prisma_1.prisma.coupon.count({
            where: whereConditions,
        });
        res.status(200).json({
            success: true,
    ...
    try {
        ...
        const coupons = await prisma_1.prisma.coupon.findMany({
            where: {
                ...
                OR: [
                    { maxUsage: null },
                    { maxUsage: { gt: prisma_1.prisma.coupon.fields.usedCount } },
                ],
            },
            select: {
                id: true,
...
const updateCoupon = async (req, res) => {
    try {
        ...
        const existingCoupon = await prisma_1.prisma.coupon.findUnique({ where: { id } });
        if (!existingCoupon) {
            return res.status(404).json({
                success: false,
                message: "Coupon not found",
        ...
        if (excludedProductIds !== undefined) {
            updateData.excludedProducts = {
                set: excludedProductIds.map((id) => ({ id })),
            };
        }
        ...
        const updatedCoupon = await prisma_1.prisma.coupon.update({
            where: { id },
            data: updateData,
            include: {
                applicableTo: true,
...
const deleteCoupon = async (req, res) => {
    try {
        ...
        if (activeOrders.length > 0) {
            return res.status(400).json({
                success: false,
                message: "Cannot delete coupon. It is currently being used in active orders.",
            });
        }
...
const validateCoupon = async (req, res) => {
    try {
        ...
        if (coupon.maxUsagePerUser > 0) {
            const userUsage = await prisma_1.prisma.order.count({
                where: {
                    userId,
                    couponId: coupon.id,
        ...
        if (new client_1.Prisma.Decimal(totalAmount).lessThan(coupon.minOrderValue)) {
            return res.status(400).json({
                success: false,
        ...
        if (cartItems && cartItems.length > 0) {
            ...
            const hasExcludedProducts = products.some((product) => coupon.excludedProducts.some((excluded) => excluded.id === product.id));
            if (hasExcludedProducts) {
                return res.status(400).json({
                    success: false,
                    message: "Some products in your cart are not eligible for this coupon.",
                });
            }
            // Check category restrictions
            if (coupon.categories.length > 0) {
                const hasValidCategory = products.some((product) => coupon.categories.some((category) => category.id === product.categoryId));
                if (!hasValidCategory) {
                    return res.status(400).json({
                        success: false,
            ...
            if (coupon.productTypeId) {
                const hasValidType = products.some((product) => product.typeId === coupon.productTypeId);
                if (!hasValidType) {
                    return res.status(400).json({
                        success: false,
                        message: "This coupon is not applicable to the product types in your cart.",
                    });
                }
            }
        }
        ...
        if (coupon.discountType === "PERCENTAGE") {
            discountAmount = new client_1.Prisma.Decimal(totalAmount)
