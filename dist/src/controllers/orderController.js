...
exports.trackOrder = exports.cancelOrder = exports.updateOrderStatus = exports.getOrderById = exports.getOrders = exports.createOrder = void 0;
...
const calculateOrderTotals = (cart) => {
    ...
    const discountAmount = cart.discountAmount || new library_1.Decimal(0);
    ...
    return {
        ...
        taxPrice,
        discountAmount,
        totalAmount: totalAmount.isNegative() ? new library_1.Decimal(0) : totalAmount,
    };
};
/**
...
const createOrder = async (req, res) => {
    try {
        ...
        const { shippingAddress, paymentMethod, couponCode, notes, adminNotes } = req.body;
        if (!userId) {
            return res
                .status(401)
        ...
        if (!finalShippingAddress) {
            ...
            const userAddress = await prisma_1.prisma.address.findFirst({
        ...
        // Validate and format shipping address
        const formattedShippingAddress = validateShippingAddress(finalShippingAddress);
...
const cancelOrder = async (req, res) => {
    try {
        ...
        const orderToCancel = await prisma_1.prisma.order.findUnique({
            where: { id },
            include: { orderItems: true },
        });
        if (!orderToCancel) {
        ...
        if (orderToCancel.orderStatus === "SHIPPED" ||
        ...
        const updatedOrder = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            const order = await tx.order.update({
                ...
                data: {
                    ...
                    paymentStatus: orderToCancel.paymentStatus === "COMPLETED" ? "REFUNDED" : "FAILED",
                    adminNotes: reason
